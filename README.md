# cpp_uni_logger
This is a simple library for adding logging and informative data output to your project.

### Include library
You can simply add files from **Include** and **Source** directories to your project to take advantage of all the library's features.

### Write message into terminal
You can output information to the console in a convenient format with quick visual identification of the message type. You can see an example in the Test directory.
![Console output](docs/images/console.png)